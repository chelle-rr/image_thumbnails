...
montage -label '%t' -font Helvetica -pointsize 10 -size 200x200 @$image_dir/thumb_list.txt -geometry 280x190 -tile "$num_columns"x $image_dir/_img-thumbnails.png

# Check if contact sheet was successfully created
if test -f $image_dir/_img-thumbnails.png; then
	echo "_img-thumbnails successfully created"
else
	echo "Error creating _img-thumbnails"
fi
